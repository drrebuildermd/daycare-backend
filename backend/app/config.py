...
class Settings(BaseSettings):
    ...
    # 하원 운행 마감 시각. 이 시각까지는 운행이 끝나야 한다.
    #
    # 이 제약이 없으면 솔버가 3회차를 저녁까지 늘어뜨린다. 그러면 조기
    # 하원이 일어나지 않아 '3회차의 비용' 이라는 것이 아예 생기지 않는다.
    # 현장에서 3회차가 비싼 이유가 바로 이 마감을 맞추려 앞당기기 때문이다.
    #
    # 차량마다 다를 수 있어 차량 쪽에서 덮어쓸 수 있다.
    outbound_deadline: str = "17:00"

    # ── 센터 주도형 배차 (v4.1) ───────────────────────────────
    #
    # 어르신 시간을 비워 두면 엔진이 알아서 정한다. 다만 무한정 여는 것이
    # 아니라 아래 세 가지로 울타리를 친다.

    # 아무리 일러도 이 시각 전에는 모시러 가지 않는다.
    # 상한만 두면 솔버가 새벽에도 태우러 간다.
    earliest_pickup: str = "07:30"

    # 한 회차가 걸리는 전체 시간의 상한(분).
    #
    # 시간 제약이 느슨해지면 차 한 대로 온 동네를 도는 답이 싸 보인다.
    # 그러면 첫 번째로 타신 분이 두 시간을 차에 계신다.
    #
    # 회차 전체를 묶는 것이라 개인 탑승 시간보다 조금 보수적이다.
    # 60분은 외곽에 사시는 분을 배차 불가로 만들 위험이 커서 80분에서 시작한다.
    max_transit_minutes: int = 80
    # 전원 배차를 위해 양보할 때 마지막까지 지킬 것.
    # 화면에서 고른 값이 이것을 이긴다. 여기 기본값은 지금까지의 동작과 같다.
    dispatch_priority: str = "revenue"

    # 수가 구간 경계에 아슬아슬하게 붙지 않도록 두는 여유(분).
    #
    # 마지노선에 딱 맞춰 배차하면 이용시간이 정확히 8시간이 된다.
    # 길이 막혀 5분만 늦어도 구간이 내려가고 수가가 깎인다.
    deadline_safety_margin_minutes: int = 15
